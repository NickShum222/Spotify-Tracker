...
const Sidebar = ({token}) => {
  ...
  return (
    <div className='bg-black h-[100vh] w-[250px] fixed flex flex-col justify-start items-center shadow-2xl z-50'>
      <h3 className="text-white font-bold text-[28px] pt-5">Spot my <span className="text-spotify">Stats</span></h3>
      <div className="flex flex-col justify-start items-start w-full pl-6">
        <p className="text-semiwhite text-[22px]">Menu</p>
        {navLinks.map((link, index) => (
          <div
            key={index}
            className={`flex flex-row justify-start items-center w-full py-2 px-4 cursor-pointer ${
              active === link.path ? "border-r-[4px] border-spotify" : ""
            }`}
            onClick={() => {
              setActive(link.path);
